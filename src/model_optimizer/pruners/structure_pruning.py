...
BACKWARD_PARSER_DICT: Dict[str, Any] = {}  # for conv, linear, concat
MAKE_GROUP_PARSER_DICT: Dict[str, Any] = {}
...
class StructurePruner(nn.Module):  # pylint: disable=missing-class-docstring,too-many-public-methods
    def __init__(self, except_start_keys=None):
        super().__init__()
        if except_start_keys is None:
            self.except_start_keys = []
        else:
            self.except_start_keys = except_start_keys

    def prepare_from_supernet(self, supernet):  # need to exec on cpu! for mb_v2,has 22 groups
        """Prepare for pruning.
        supernet: not wrapped by ddp
        """
        ...
        supernet = supernet.cpu()  # trace parents should in cpu

        for name, module in supernet.named_modules():
            if hasattr(module, 'weight'):
                module2name[module] = name
                name2module[name] = module   # only conv, linear, bn
                var2module[id(module.weight)] = module
                self.add_pruning_attrs(module)  # add mask
                visited[name] = False
        self.name2module = name2module  # pylint: disable=attribute-defined-outside-init

        pseudo_img = torch.randn(1, 3, 224, 224)
        pseudo_img = supernet(pseudo_img)  # pseudo_img.shape [1, 1000]
        pseudo_loss = self.cal_pseudo_loss(pseudo_img)  # pseudo_loss.shape [1000]

        non_pass_paths = []   # conv, linear, concat
        cur_non_pass_path = []
        ...
        self.bn_conv_links = bn_conv_links  # pylint: disable=attribute-defined-outside-init

        # a node can be the name of a conv module or a str like 'concat_{id}'
        node2parents = self.find_node_parents(non_pass_paths)  # conv, linear, concat
        self.node2parents = node2parents  # pylint: disable=attribute-defined-outside-init
        same_out_channel_groups = self.make_same_out_channel_groups(
            node2parents, name2module)

        self.module2group = {}  # pylint: disable=attribute-defined-outside-init
        for group_name, group in same_out_channel_groups.items():
            for module_name in group:
                self.module2group[module_name] = group_name

        self.modules_have_ancest = []  # pylint: disable=attribute-defined-outside-init
        for node_name, parents_name in node2parents.items():
            if node_name in name2module and len(parents_name) > 0:
                self.modules_have_ancest.append(node_name)  # only conv,linear

        self.modules_have_child = OrderedSet()  # pylint: disable=attribute-defined-outside-init
        for parents_name in node2parents.values():
            for name in parents_name:
                # The node is a module in supernet
                if name in name2module:
                    self.modules_have_child.add(name)  # # only conv,linear
        # build channel spaces , only conv,linear
        self.channel_spaces = self.build_channel_spaces(name2module)  # pylint: disable=attribute-defined-outside-init

        self.input_layers = []  # pylint: disable=attribute-defined-outside-init
        for name, module in supernet.named_modules():
            if name not in self.modules_have_ancest and name in self.modules_have_child:
                self.input_layers.append(name)

        supernet.to(device)  # set back to original device
    ...
    def trace_bn_conv_links(self, grad_fn, module2name, var2module,
        ...
        Example:
            ...
            >>> # ThnnConv2DBackward
            >>> # op.next_functions[1][0].variable is the weight of this bn module
            >>> # op.next_functions[2][0].variable is the bias of this bn module
        """
    ...
    def add_pruning_attrs(self, module):
        ...
        if isinstance(module, nn.modules.batchnorm._BatchNorm):  # pylint: disable=protected-access
            module.register_buffer(
                'out_mask',
    ...
    def trace_non_pass_path(self, grad_fn, module2name, var2module, cur_path,
        ...
        if grad_fn is not None:
            parser = self._find_backward_parser(grad_fn)
    ...
    def _find_backward_parser(self, grad_fn):
    ...
    def conv_backward_parser(self, grad_fn, module2name, var2module, cur_path,
        ...
        Example:
            ...
            >>> # op.next_functions[0][0] is None means this ThnnConv2DBackward op has no parents
            >>> # op.next_functions[1][0].variable is the weight of this Conv2d module
            >>> # op.next_functions[2][0].variable is the bias of this Conv2d module
        """
    ...
    def find_node_parents(self, paths):
        ...
        Args:
            paths (list): The traced paths (non_pass_path).
        """
        node2parents = {}
        for path in paths:
            if len(path) == 0:
            ...
            for i, node_name in enumerate(path[:-1]):
                ...
                if node_name in node2parents.keys():  # pylint: disable=consider-iterating-dictionary
                    node2parents[node_name].add(parent_name)
                else:
                    node2parents[node_name] = OrderedSet([parent_name])

            leaf_name = path[-1]
            if leaf_name not in node2parents.keys():  # pylint: disable=consider-iterating-dictionary
    ...
    def linear_backward_parser(self, grad_fn, module2name, var2module,
        ...
        """Parse the backward of a linear layer.
        ...
        Example:
            ...
            >>> # op.next_functions[0][0].variable is the bias of this Linear module
            >>> # op.next_functions[1][0] is None means this AddmmBackward op has no parents
    ...
    def find_make_group_parser(self, node_name, name2module):
        ...
        elif (node_name in name2module and isinstance(name2module[node_name], nn.Conv2d)
    ...
    # error here, chunk_make_group_parser should have the same signature as other parser
    ...
    # output module's space_id in None, cos it's not be pruned
    def get_space_id(self, module_name):
        ...
        if 'concat' in module_name and module_name not in self.name2module:
            ...
            if 'item' in module_name:
                ...
                space_id = {"concat": concat_parents}
    ...
    #  pay attention to supported module type
    def set_subnet(self, subnet_dict):
        # in subnet_dict keys, not include except_start_keys layers
        ...
        """
        for module_name in self.modules_have_child:
            # bug: modules_have_child only for mb_v2: conv2d,
            # NON_PASS_MODULE = (nn.Conv2d, nn.Linear).
            # in except_start_keys include conv2d, this may be wrong,
            # cause subnet_dict keys, not include except_start_keys layers
        ...
        for module_name in self.modules_have_ancest:
            ...
            # In case 1, all the ``Conv2d`` share the same space_id and out_mask.
    ...
    @staticmethod
    def deploy_subnet(supernet, channel_cfg):  # pylint: disable=too-many-branches
    ...
    def get_max_channel_bins(self, max_channel_bins):
        ...
        """
        channel_bins_dict = {}
        for space_id, _ in self.channel_spaces.items():
            channel_bins_dict[space_id] = torch.ones((max_channel_bins, ))
        return channel_bins_dict

    def set_channel_bins(self, channel_bins_dict, max_channel_bins, align=False, round_nearest=8):
        ...
        Args:
            channel_bins_dict (dict): The number of bins in each layer. Key is
                ...
                mask of channel bin.
            max_channel_bins (int): The max number of bins in each layer.
            align(boolean): is align channels
            round_nearest(int): align channel num
        """
        subnet_dict = {}
        for space_id, bin_mask in channel_bins_dict.items():
            ...
            if align:
                out_channels = new_mask.sum()
                out_channels_align = make_divisible(out_channels, round_nearest)
                new_mask[:, 0:out_channels_align, :, :] = 1
                new_mask[:, out_channels_align:, :, :] = 0
