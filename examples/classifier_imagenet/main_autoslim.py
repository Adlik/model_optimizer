...
"""
autoslim train/validate on multi gpu
"""
import copy
import time
import datetime
...
from model_optimizer.distillers.self_distiller import SelfDistiller
from model_optimizer.losses.kl_divergence import KLDivergence
...
def main_worker(gpu, ngpus_per_node, args):  # pylint: disable=too-many-branches,too-many-statements
    ...
    kl_divergence = KLDivergence(temperature=args.hp.distill.kl_divergence.temperature,
                                 reduction=args.hp.distill.kl_divergence.reduction,
                                 loss_weight=args.hp.distill.kl_divergence.loss_weight)
    distiller = SelfDistiller(kl_divergence)
    algorithm = AutoSlim(model, pruner, distiller, bn_training_mode=args.hp.auto_slim.bn_training_mode,
    ...
    for epoch in range(args.start_epoch, args.hp.epochs):
        ...
        if args.hp.auto_slim:
            algorithm.train_epoch(train_loader, criterion, optimizer, epoch, args, writer, scaler)
            lr_scheduler.step()
        else:
        ...
        if args.hp.auto_slim:
            acc1, acc5 = validate(val_loader, model, criterion, args, algorithm)
        else:
            acc1, acc5 = validate(val_loader, model, criterion, args)
        if writer is not None:
            writer.add_scalar('val/acc1', acc1, epoch + 1)
            writer.add_scalar('val/acc5', acc5, epoch + 1)
            writer.add_scalar('val/lr', optimizer.param_groups[0]['lr'], epoch + 1)
